...
<body>
            ...
            <div class="container">
                ...
                <nav class="nav-menu mobile-menu">
                    <ul>
                        ...
                        <li class="active"><a href="recomendado.php">Recomendado para ti</a></li>
    ...
    <div class="container">
  <h2>Productos para tratar Lesiones</h2>
  ...
  <table class="table table-bordered table-striped">
    ...
    $variable = "SELECT Linea_producto, categoría_producto
        FROM (SELECT clave_producto, sum(cantidad) as cantidad
        FROM (SELECT clave_producto, cantidad
            FROM (select a.id_persona, correo from (select o.id_persona, nombre_persona, apellido_persona, 
            fechaNacimiento_persona, ciudad_persona, genero_persona, correo, id_teléfono, teléfono from 
            (select Persona.id_persona, nombre_persona, apellido_persona, fechaNacimiento_persona, 
                ciudad_persona, genero_persona, correo from Persona inner join Correos on Persona.id_persona=Correos.id_persona) as
                 o inner join Teléfonos on o.id_persona = Teléfonos.id_persona) as a inner join Cliente on a.id_persona = Cliente.id_persona where 
        correo='".$email."') as os INNER JOIN Venta
        ON os.id_persona = Venta.id_cliente) as new
        GROUP BY clave_producto ORDER BY cantidad DESC limit 1) as ult
        INNER JOIN Producto ON ult.clave_producto = Producto.clave_producto";
       ...
       // echo $tabla->num_rows;
        while($row = $tabla->fetch_assoc()) {
          //  var_dump($row);
            ...
            $categoria=$row["categoría_producto"];
        }
...
$sql1="select * from Producto where categoría_producto='".$categoria."' limit 10";
$sql2="select * from Producto where Linea_producto='".$linea."' limit 3";


      $result2 = $baseDatos->query($sql2);
      $result1 = $baseDatos->query($sql1);

      if ($result2->num_rows > 0) {
        // output data of each row
        while($row = $result2->fetch_assoc()) {
          echo "<tr>  <td class='td1'>  <img src='".$row["imagen"]."'/ width='550' height='250'> 
