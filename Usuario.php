    ...
    <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
      <div class="sidebar-sticky pt-3">
        <ul class="nav flex-column">
          ...
          <li class="nav-item">
            <a class="nav-link active" href="http://localhost/Usuario.php">
...
<div class="container">
  ...
  <table class="table table-bordered table-striped">
        <th>id</th> <th>Nombre</th> <th>Apellido</th> <th>Fecha de Nacimiento</th><th>Ciudad</th><th>Genero</th><th>Correo</th><th>id_teléfono</th><th>Teléfono</th>
    <tbody id="myTable">
      <?php
        $sql = "select o.id_persona, nombre_persona, apellido_persona, fechaNacimiento_persona, ciudad_persona, genero_persona, correo, id_teléfono, teléfono from (select Persona.id_persona, nombre_persona, apellido_persona, fechaNacimiento_persona, ciudad_persona, genero_persona, correo from Persona inner join Correos on Persona.id_persona=Correos.id_persona) as o inner join Teléfonos on o.id_persona = Teléfonos.id_persona;";
      $result = $baseDatos->query($sql);

      if ($result->num_rows > 0) {
        // output data of each row
        while($row = $result->fetch_assoc()) {
          echo "<tr><td>".$row["id_persona"]."</td><td>".$row["nombre_persona"]."</td><td>".$row["apellido_persona"]."</td><td>".$row["fechaNacimiento_persona"]."</td><td>".$row["ciudad_persona"]."</td><td>".$row["genero_persona"]."</td><td>".$row["correo"]."</td><td>".$row["id_teléfono"]."</td><td>".$row["teléfono"]."</td></tr>";
        }
      } else {
        echo "0 results";
      }
  ...
  <p>No se encontraron Usuarios con esa descripción.</p>
</div>
