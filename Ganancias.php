...
<div class="container">
  <h2>Salario de los vendedores:</h2>
 ...
 <th>Nombre</th> <th>Apellido</th> <th>Salario</th>
    <tbody id="myTable">
      <?php
        $sql = "SELECT nombre_persona, apellido_persona, salario FROM Vendedor
        INNER JOIN Persona
        ON Persona.id_persona = Vendedor.id_persona";
      $result = $baseDatos->query($sql);

      if ($result->num_rows > 0) {
        // output data of each row
        while($row = $result->fetch_assoc()) {
          echo "<tr><td>".$row["nombre_persona"]."</td><td>".$row["apellido_persona"]."</td><td>".$row["salario"]."</td></tr>";
        }
      } else {
        echo "0 results";
      }
