...
<div class="container">
  <br><br>
  <h2> Top 5 en Ventas:</h2>
  <input class="form-control" id="myInput" type="text" placeholder="Buscar vendedor:">
  <br>
  <table class="table table-bordered table-striped">
      <thead class="thead-dark">
 <th>posición</th><th>Nombre</th> <th>Apellido</th> <th>Ventas</th>
    <tbody id="myTable">
      <?php
        $sql = "select nombre_persona, apellido_persona, Ventas
          from (SELECT id_vendedor, sum(cantidad*costo_producto) as Ventas
          FROM (select id_vendedor, cantidad, costo_producto from Producto INNER JOIN Venta ON Producto.clave_producto=Venta.clave_producto) as a
          GROUP BY id_vendedor
          ORDER BY Ventas DESC limit 5) as os
          inner join Persona
          ON Persona.id_persona = os.id_vendedor";
      $result = $baseDatos->query($sql);
      $var=1;
      if ($result->num_rows > 0) {
        // output data of each row
        while($row = $result->fetch_assoc()) {
          echo "<tr><td>$var</td><td>".$row["nombre_persona"]."</td><td>".$row["apellido_persona"]."</td><td>".$row["Ventas"]."</td></tr>";
          $var++;
        }
      } else {
        echo "0 results";
