...
<div class="container">
 ...
 <th>posición</th><th>Nombre</th> <th>Apellido</th> <th>Compras</th>
    <tbody id="myTable">
      <?php
        $sql = "select nombre_persona, apellido_persona, Compras
      from (SELECT id_cliente, sum(cantidad*costo_producto) as Compras
      FROM (select id_cliente, cantidad, costo_producto from Producto INNER JOIN Venta ON Producto.clave_producto=Venta.clave_producto) as a
      GROUP BY id_cliente
      ORDER BY Compras DESC limit 5) as os
      inner join Persona
      ON Persona.id_persona = os.id_cliente";
      $result = $baseDatos->query($sql);
      $var=1;
      if ($result->num_rows > 0) {
        // output data of each row
        while($row = $result->fetch_assoc()) {
          echo "<tr><td>$var</td><td>".$row["nombre_persona"]."</td><td>".$row["apellido_persona"]."</td><td>".$row["Compras"]."</td></tr>";
          $var++;
        }
      } else {
        echo "0 results";
