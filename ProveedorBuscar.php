...
<div class="container">
  <h2>Proveedores:</h2>
  <input class="form-control" id="myInput" type="text" placeholder="Buscar proveedor:">
  <br>
  <table class="table table-bordered table-striped">
        <th>id</th> <th>Nombre</th> <th>Apellido</th> <th>Fecha de Nacimiento</th><th>Ciudad</th><th>Genero</th><th>Correo</th><th>id_teléfono</th><th>Teléfono</th><th>Empresa</th> 
    <tbody id="myTable">
      <?php
        $sql = "select a.id_persona, nombre_persona, apellido_persona, fechaNacimiento_persona, ciudad_persona, genero_persona, correo, id_teléfono, teléfono, empresa from (select o.id_persona, nombre_persona, apellido_persona, fechaNacimiento_persona, ciudad_persona, genero_persona, correo, id_teléfono, teléfono from (select Persona.id_persona, nombre_persona, apellido_persona, fechaNacimiento_persona, ciudad_persona, genero_persona, correo from Persona inner join Correos on Persona.id_persona=Correos.id_persona) as o inner join Teléfonos on o.id_persona = Teléfonos.id_persona) as a inner join Proveedor on a.id_persona = Proveedor.id_persona;";
      $result = $baseDatos->query($sql);

      if ($result->num_rows > 0) {
        // output data of each row
        while($row = $result->fetch_assoc()) {
          echo "<tr><td>".$row["id_persona"]."</td><td>".$row["nombre_persona"]."</td><td>".$row["apellido_persona"]."</td><td>".$row["fechaNacimiento_persona"]."</td><td>".$row["ciudad_persona"]."</td><td>".$row["genero_persona"]."</td><td>".$row["correo"]."</td><td>".$row["id_teléfono"]."</td><td>".$row["teléfono"]."</td><td>".$row["empresa"]."</td></tr>";
        }
      } else {
        echo "0 results";
      }
